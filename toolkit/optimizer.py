...
def get_optimizer(
    ...
    if lower_type.startswith("dadaptation"):
        ...
        if use_lr < 0.1:
            use_lr = 1.0
        print(f"Using lr {use_lr}")
        optimizer = ProdigyPlusScheduleFree(params, lr=use_lr, eps=1e-8, **optimizer_params)
        ...
        # let net be the neural network you want to train
        # you can choose weight decay value based on your problem, 0 by default
        optimizer = Prodigy(params, lr=use_lr, eps=1e-8, **optimizer_params)
